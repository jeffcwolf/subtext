...
Usage:
    python pipeline/explore_glopardo.py          # or: uv run python ...
    SUBTEXT_SAMPLE=2000 python pipeline/explore_glopardo.py
"""
...
def main() -> int:
    ...
    period_cols = [
        c for c in cols if looks_like(c, ("quarter", "fiscal", "period", "year", "qtr"))
    ]
    fin_cols = [
        c
        for c in cols
    ...
    text_cols = [
        c for c in cols if looks_like(c, ("transcript", "text", "content", "body"))
    ]
    ...
    for c in period_cols:
        ...
        print(
            f"  {c}: {dict(sorted(cnt.items())[:12])}" + (" …" if len(cnt) > 12 else "")
        )

    header("FINANCIAL COLUMNS (the supplement value)")
    if not fin_cols:
        print(
            "  No obviously-financial columns by name — inspect the sample "
    ...
    for c in fin_cols:
        ...
        if vals:
            ...
            print(
                f"  {c:<20} type={ds.features[c]}  n={len(vals):,} nulls={nulls:,}  "
